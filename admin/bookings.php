...
$statusCounts = [];
foreach (BOOKING_STATUSES as $s) {
    $statusCounts[$s] = count(array_filter($all, fn($b) => $b['status'] === $s));
}
...
<div class="status-tabs">
  ...
  <?php foreach (BOOKING_STATUSES as $s): ?>
    <a href="?status=<?= $s ?>" class="tab <?= $filterStatus === $s ? 'active' : '' ?>"><?= $s ?> <span class="tab-count"><?= $statusCounts[$s] ?></span></a>
  <?php endforeach; ?>
</div>
  ...
  <div class="table-wrap">
    ...
    <?php else: ?>
      <table class="data-table">
        <thead>
          <tr>
            <th>#</th><th>Client</th><th>Company</th><th>Event Type</th>
            <th>Event Date</th><th>Package</th><th>Duration</th>
